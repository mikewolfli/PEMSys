		...
		for (col = minVisibleCol; col <= maxVisibleCol; col++)
		{
			...
			if (pCell)
			{
				...
				if(!pCell->IsMerged())
				{
					...
					{
						CRect rtNonMerge = rect;            
						rtNonMerge.DeflateRect(0,0,1,1);
						...
						if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_VERT)  
						{                                                         
						pDC->MoveTo(rtNonMerge.right+1, rtNonMerge.top);        
						pDC->LineTo(rtNonMerge.right+1, rtNonMerge.bottom+1);   
						}                                                          
						if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_HORZ)  
						{                                                          
						pDC->MoveTo(rtNonMerge.left, rtNonMerge.bottom+1);      
						pDC->LineTo(rtNonMerge.right+1, rtNonMerge.bottom+1);   
						} 
					}
					else
					{
						...
						if(GetCellRangeRect(pMergedCell->m_MergeRange,&mergerect))
						{
							...
							if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_VERT)  
							{                                                          
							pDC->MoveTo(mergerect.right+1, mergerect.top);          
							pDC->LineTo(mergerect.right+1, mergerect.bottom+1);     
							}                                                          
							if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_HORZ)  
							{                                                          
							pDC->MoveTo(mergerect.left, mergerect.bottom+1);        
							pDC->LineTo(mergerect.right+1, mergerect.bottom+1);     
							}                                                          
						}
					}
...
//                    pDC->LineTo(rect.right, rect.top);
 ...
 //                   pDC->LineTo(rect.left, rect.bottom);
  ...
  //          }
	...
	//change as victor_zhao said
//	white_rect.top=range.bottom+1;
	white_rect.top=range.bottom+2;
