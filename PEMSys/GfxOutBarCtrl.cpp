...
void CGfxOutBarCtrl::OnDestroy() 
{
	...
	if(m_hBkFolder)
	{
		g_af.BkDelete(m_hBkFolder) ;
		m_hBkFolder = NULL ;
	}
	if(m_hArrow)
	{
		g_af.BkDelete(m_hArrow) ;
		m_hArrow = NULL ;
	}
...
long CGfxOutBarCtrl::OnEndLabelEdit(WPARAM wParam, LPARAM lParam)
{
	if (wParam == NM_OB_ONGROUPENDEDIT)
	{
		...
		if (GetOwner()->PostMessage(WM_OUTBAR_NOTIFY, NM_OB_ONGROUPENDEDIT, (LPARAM) &ob))
		{
	...
	if (wParam == NM_OB_ONLABELENDEDIT)
	{
		...
		if (GetOwner()->PostMessage(WM_OUTBAR_NOTIFY, NM_OB_ONLABELENDEDIT, (LPARAM) &ob))
		{
	...
	if (iLastSel >= iFirstItem && irc.bottom > rc.bottom && irc.top < rc.top)
	{
		...
		if (IsSmallIconView())
		{
			if (ima)
			{
				...
				if(!g_af.BkDraw(m_hBk,(HDC)dc,&rcBck,0))
...
void CGfxOutBarCtrl::OnPaint() 
{
	...
	if (!GetFolderChild()) 
	{
		...
		if(!g_af.BkDraw(m_hBk,pDC->GetSafeHdc(),&rc,0))
	...
	if (!GetFolderChild())
	{
		...
		if (f > 0)
		{
			if (bUpPressed) 
			{
				if(!g_af.BkDraw(m_hArrow,pDC->GetSafeHdc(),&rcUpArrow, ARROW_UP_DOWN))
					pDC->DrawFrameControl(rcUpArrow, DFC_SCROLL, DFCS_SCROLLUP|DFCS_PUSHED);
			}
			else
			{
				if(!g_af.BkDraw(m_hArrow,pDC->GetSafeHdc(),&rcUpArrow, ARROW_UP_NORMAL))
		...
		if (l < GetItemCount() - 1)
		{
			if (bDownPressed)
			{
				if(!g_af.BkDraw(m_hArrow,pDC->GetSafeHdc(),&rcDownArrow, ARROW_DOWN_DOWN))
					pDC->DrawFrameControl(rcDownArrow, DFC_SCROLL, DFCS_SCROLLDOWN|DFCS_PUSHED);
			}
			else 
			{
				if(!g_af.BkDraw(m_hArrow,pDC->GetSafeHdc(),&rcDownArrow, ARROW_DOWN_NORMAL))
...
void CGfxOutBarCtrl::DrawFolder(CDC * pDC, const int iIdx, CRect rect, const bool bSelected)
{
	...
	if (!bSelected)
	{
		...
		if(!g_af.BkDraw(m_hBkFolder,pDC->GetSafeHdc(),&rect,0))
...
void CGfxOutBarCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
				...
				case WM_LBUTTONUP:
					{
						...
						if (!bDragging)
						{
							if (ht1 == htItem && idx == index) 
							{
								...
								GetOwner()->PostMessage(WM_OUTBAR_NOTIFY, NM_OB_ITEMCLICK, idx);
							}
						}
						else
						{
							if (ht1 == htItem)
							{
								if (idx != index)
								{
									...
									ob.iDragTo = index;
									if (GetOwner()->PostMessage(WM_OUTBAR_NOTIFY, NM_OB_DRAGITEM, (LPARAM) &ob))
									{
										CBarFolder * pbf = (CBarFolder *) arFolder.GetAt(iSelFolder);
	...
	if (ht == htDownScroll)
	{
		...
		if (::GetCapture() == NULL)
		{
			...
			if(!g_af.BkDraw(m_hArrow,dc.GetSafeHdc(),&rcDownArrow, ARROW_DOWN_DOWN))
				...
				case WM_MOUSEMOVE:
					{
						...
						if (rcDownArrow.PtInRect(pt))
						{
							if (bHigh == false)
							{
								if(!g_af.BkDraw(m_hArrow,(HDC)dc,&rcDownArrow, ARROW_DOWN_DOWN))
									dc.DrawFrameControl(rcDownArrow, DFC_SCROLL, DFCS_SCROLLDOWN|DFCS_PUSHED);
						...
						else
						{
							if (bHigh == true)
							{
								if(!g_af.BkDraw(m_hArrow,(HDC)dc,&rcDownArrow, ARROW_DOWN_NORMAL))
	...
	if (ht == htUpScroll)
	{
		...
		if (::GetCapture() == NULL)
		{
			...
			if(!g_af.BkDraw(m_hArrow,(HDC)dc,&rcUpArrow, ARROW_UP_DOWN))
				...
				case WM_MOUSEMOVE:
					{
						...
						else
						{
							if (bHigh == true)
							{
								if(!g_af.BkDraw(m_hArrow,(HDC)dc,&rcUpArrow, ARROW_UP_NORMAL))
...
//	if (index == iSelFolder) return;
	...
	GetOwner()->PostMessage(WM_OUTBAR_NOTIFY, NM_FOLDERCHANGE, (LPARAM) iSelFolder);
...
void CGfxOutBarCtrl::HighlightItem(const int index, const bool bPressed)
{
	...
	if (iLastItemHighlighted >= 0 && IsValidItem(iLastItemHighlighted))
	{
		...
		if(m_hBk)
		{
			g_af.BkDraw( m_hBk ,dc.m_hDC, &rc ,0); 
			rc.DeflateRect(1,1);
...
void CGfxOutBarCtrl::AnimateFolderScroll(const int iFrom, const int iTo)
{
	...
	if (iTo > iFrom)
	{
		...
		if(!g_af.BkDraw(m_hBk,pDC->GetSafeHdc(),&rc1,0))
...
void CGfxOutBarCtrl::SkinReset()
{
	...
	m_hBk = g_af.BkCreate(BK_DIALOGPANEL) ;
	m_hBkFolder = g_af.BkCreate(BK_MENUBARBK) ;
	m_hArrow = g_af.BkCreate(BK_IMAGE_ARROW) ;
}
