...
CAutoHideBar::~CAutoHideBar()
{
	if(m_hBk) {g_af.BkDelete(m_hBk) ; m_hBk = NULL ;}
...
void CAutoHideBar::Draw(CDC * pDC)
{
	...
	GetClientRect(&rt);
	CRect txtRt;
	if(!g_af.BkDraw(m_hBk,pDC->GetSafeHdc(),&rt,0))
...
BOOL CAutoHideBar::OnEraseBkgnd(CDC* pDC) 
{
	// TODO: Add your message handler code here and/or call default
	return TRUE ;
...
void CAutoHideBar::SkinReset()
{
	HANDLE h  = NULL;

	h = m_hBk ;
	m_hBk =NULL ;
	if(h)  g_af.BkDelete(h) ;
